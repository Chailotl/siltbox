...
public class PailItem extends BucketItem
{
				...
				if (fluid == Fluids.EMPTY)
				{
					...
					if (block instanceof FluidDrainable && block.getFluidState(blockState) != Fluids.LAVA.getStill(false))
					{
						...
						if (fluid != Fluids.EMPTY)
						{
							...
							ItemStack itemStack2 = ItemUsage.method_30012(itemStack, user, new ItemStack(Main.WATER_PAIL));
							if (!world.isClient)
							{
								Criteria.FILLED_BUCKET.trigger((ServerPlayerEntity)user, new ItemStack(Main.WATER_PAIL));
							}
	...
	{
		return !player.abilities.creativeMode ? new ItemStack(Main.PAIL) : stack;
	}
